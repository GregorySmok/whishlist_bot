...
async def set_default_keyboard(chat_id):
    try:
        ...
        await bot.send_message(chat_id, "Произошла ошибка при настройке клавиатуры. Пожалуйста, попробуйте позже или используйте текстовые команды.")
...
async def start_handler(message: Message, state: FSMContext):
    try:
        if not message.from_user.username:
            ...
            log_user_action(message.from_user.id, "no_username", "start_attempt",
        ...
        if message.from_user.id not in users_list:
            ...
            log_user_action(message.from_user.id, message.from_user.username,
                            "returning_user_login", "Existing user started bot")
        ...
        await bot.send_message(message.from_user.id, "Произошла ошибка при запуске бота. Пожалуйста, попробуйте позже или обратитесь к администратору.")
...
async def add_item_handler(message: Message, state: FSMContext):
    try:
        ...
        log_user_action(message.from_user.id, message.from_user.username,
                        "add_item_started", "User initiated adding item")
    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(message.from_user.id,
                  f"Error in add_item_handler: {e}", error_traceback)
        await bot.send_message(message.from_user.id, "Произошла ошибка при попытке добавить товар. Пожалуйста, попробуйте позже.")
        await state.set_state(States.already_started)
        await set_default_keyboard(message.from_user.id)
...
async def adding_item(message: Message, state: FSMContext):
    try:
        ...
        if item_link == "0":
            ...
            await set_default_keyboard(message.from_user.id)
            log_user_action(message.from_user.id, message.from_user.username,
                            "add_item_cancelled", "User cancelled adding item")
            return
        ...
        if await check_link_liquidity(item_link):
            if item_link in goods:
                ...
                log_user_action(message.from_user.id, message.from_user.username,
                                "add_item_duplicate", f"User tried to add duplicate link: {item_link}")
                return
            ...
            log_user_action(message.from_user.id, message.from_user.username,
                            "add_item_success", f"Added item: {item_link}")

            await state.set_state(States.already_started)
            await set_default_keyboard(message.from_user.id)
        else:
            await bot.send_message(message.from_user.id, "Ошибка: ссылка не найдена или недоступна. Пожалуйста, убедитесь, что ссылка начинается с 'https://' и попробуйте снова.")
            log_user_action(message.from_user.id, message.from_user.username,
                            "add_item_invalid_link", f"User tried to add invalid link: {item_link}")

    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(message.from_user.id,
                  f"Error in adding_item: {e}", error_traceback)
        await bot.send_message(message.from_user.id, "Произошла ошибка при добавлении товара. Пожалуйста, проверьте формат ссылки и попробуйте снова.")
        await state.set_state(States.already_started)
        await set_default_keyboard(message.from_user.id)
...
async def delete_item(callback_query: CallbackQuery, state: FSMContext):
    try:
        ...
        if 0 <= item_index < len(wishes):
            ...
            await db.execute(f"DELETE FROM {username.lower()} WHERE stuff_link = %s", (deleted_item, ))

            await callback_query.answer("Товар удален")
            await callback_query.message.delete()
            log_user_action(callback_query.from_user.id, username,
                            "delete_item_success", f"Deleted item: {deleted_item}")
        else:
            await callback_query.answer("Товар не найден")
            log_user_action(callback_query.from_user.id, username,
                            "delete_item_not_found", f"Item index out of range: {item_index}")

    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(callback_query.from_user.id,
                  f"Error while deleting item: {e}", error_traceback)
        await callback_query.answer("Произошла ошибка при удалении")
        await bot.send_message(callback_query.from_user.id, "Не удалось удалить товар. Пожалуйста, попробуйте позже.")
...
async def stop_deleting(callback_query: CallbackQuery, state: FSMContext):
    try:
        ...
        log_user_action(callback_query.from_user.id, callback_query.from_user.username,
                        "delete_item_stopped", "User finished deleting items")
    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(callback_query.from_user.id,
                  f"Error in stop_deleting: {e}", error_traceback)
        await bot.send_message(callback_query.from_user.id, "Произошла ошибка. Пожалуйста, попробуйте позже.")
        await state.set_state(States.already_started)
        await set_default_keyboard(callback_query.from_user.id)
...
async def deleting_item_handler(message: Message, state: FSMContext):
    try:
        ...
        log_user_action(message.from_user.id, message.from_user.username,
                        "delete_item_started", "User initiated deleting items")
        ...
        if not wishes:
            ...
            await set_default_keyboard(message.from_user.id)
            log_user_action(message.from_user.id, message.from_user.username,
                            "delete_item_empty_list", "User tried to delete from empty wishlist")
            return
        ...
        error_traceback = traceback.format_exc()
        log_error(message.from_user.id,
                  f"Error in deleting_item_handler: {e}", error_traceback)
        await bot.send_message(
            message.from_user.id,
...
async def my_wishlist_handler(message: Message):
    try:
        ...
        if not wishlist:
            ...
            await set_default_keyboard(message.from_user.id)
            log_user_action(message.from_user.id, message.from_user.username,
                            "view_empty_wishlist", "User viewed empty wishlist")
            return
        ...
        log_user_action(message.from_user.id, message.from_user.username,
                        "view_wishlist", f"User viewed wishlist with {len(wishlist)} items")

    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(message.from_user.id,
                  f"Error in my_wishlist_handler: {e}", error_traceback)
        await bot.send_message(message.from_user.id, "Произошла ошибка при загрузке вишлиста. Пожалуйста, попробуйте позже.")
        await set_default_keyboard(message.from_user.id)
...
async def cmd_friends(message: Message):
    try:
        ...
        log_user_action(message.from_user.id, message.from_user.username,
                        "view_friends_list", "User viewed friends list")
    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(message.from_user.id,
                  f"Error in cmd_friends: {e}", error_traceback)
        await bot.send_message(message.from_user.id, "Произошла ошибка при загрузке списка друзей. Пожалуйста, попробуйте позже.")
        await set_default_keyboard(message.from_user.id)
...
async def show_friends_page(message: types.Message, user_id: int, page: int):
    try:
        ...
        if not friends:
            if page == 0:
                await message.answer("У вас пока нет друзей. Добавьте друга, используя команду /добавить_друга")
                log_user_action(user_id, (await db.fetch_one("SELECT username FROM users WHERE id = %s", (user_id,)))[0],
                                "view_empty_friends_list", "User viewed empty friends list")
                return
            else:
        ...
        log_user_action(user_id, (await db.fetch_one("SELECT username FROM users WHERE id = %s", (user_id,)))[0],
                        "view_friends_page", f"User viewed friends page {page + 1} with {len(friends)} friends")

    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(user_id, f"Error in show_friends_page: {e}", error_traceback)
        await message.answer("Произошла ошибка при загрузке списка друзей. Пожалуйста, попробуйте позже.")
...
async def process_friend_selection(callback_query: types.CallbackQuery):
    try:
        ...
        if not wishes:
            ...
            await set_default_keyboard(callback_query.from_user.id)
            log_user_action(callback_query.from_user.id, callback_query.from_user.username,
                            "view_friend_empty_wishlist", f"User viewed empty wishlist of friend @{friend_name}")
            await callback_query.answer()
            return
        ...
        log_user_action(callback_query.from_user.id, callback_query.from_user.username,
                        "view_friend_wishlist", f"User viewed wishlist of friend @{friend_name} with {len(wishes)} items")
        ...
        error_traceback = traceback.format_exc()
        log_error(callback_query.from_user.id,
                  f"Error in process_friend_selection: {e}", error_traceback)
        await callback_query.answer("Произошла ошибка")
        await bot.send_message(callback_query.from_user.id, "Произошла ошибка при загрузке вишлиста друга. Пожалуйста, попробуйте позже.")
...
async def process_page_selection(callback_query: types.CallbackQuery):
    try:
        ...
        await callback_query.answer()
        log_user_action(callback_query.from_user.id, callback_query.from_user.username,
                        "navigate_friends_page", f"User navigated to friends page {page + 1}")

    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(callback_query.from_user.id,
                  f"Error in process_page_selection: {e}", error_traceback)
        await callback_query.answer("Произошла ошибка при навигации")
        await bot.send_message(callback_query.from_user.id, "Произошла ошибка при переходе на другую страницу. Пожалуйста, попробуйте позже.")
...
async def add_friend_handler(message: Message, state: FSMContext):
    try:
        ...
        log_user_action(message.from_user.id, message.from_user.username,
                        "add_friend_started", "User initiated adding friend")
    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(message.from_user.id,
                  f"Error in add_friend_handler: {e}", error_traceback)
...
async def adding_friend(message: Message, state: FSMContext):
    try:
        ...
        if not message.text.startswith("@"):
            ...
            log_user_action(message.from_user.id, message.from_user.username,
                            "invalid_input", f"entered invalid username format: {message.text}")
            return
        ...
        if username not in users_list:
            ...
            log_user_action(message.from_user.id, message.from_user.username,
                            "invalid_input", f"tried to add non-existent user: {message.text}")
            return
        ...
        if friend_id == message.from_user.id:
            ...
            log_user_action(message.from_user.id, message.from_user.username,
                            "invalid_input", "tried to add themselves as friend")
            return
        ...
        if old_request:
            if old_request[0] == 'accepted':
                ...
                log_user_action(message.from_user.id, message.from_user.username,
                                "invalid_input", f"tried to add existing friend: {message.text}")
                return
            ...
            if old_request[0] in ('waiting', 'rejected'):
                ...
                if time_since_request < 5:
                    ...
                    log_user_action(message.from_user.id, message.from_user.username, "invalid_input",
                                    f"tried to send friend request too soon to: {message.text}")
                    return
                else:
                    await db.execute("DELETE FROM friends_list WHERE user_1 IN (%s, %s) AND user_2 IN (%s, %s)",
                                     (user1, user2, user1, user2))
                    logger.info(
                        f"Deleted old friend request between {user1} and {user2}")

        # Создаем новый запрос дружбы
        await db.execute("INSERT INTO friends_list VALUES(%s, %s, 'waiting', %s)",
        ...
        await bot.send_message(message.chat.id, "Произошла ошибка при добавлении друга. Попробуйте позже.")
...
async def accept_friend(callback_query: CallbackQuery):
    try:
        ...
        await db.execute("UPDATE friends_list SET status = 'accepted' WHERE user_1 IN (%s, %s) AND user_2 IN (%s, %s)",
        ...
        log_user_action(
            user_id, f"accepted friend request from @{friend_name}")
...
async def reject_friend(callback_query: CallbackQuery):
    try:
        ...
        # Обновление статуса дружбы
        await db.execute("UPDATE friends_list SET status = 'rejected' WHERE user_1 IN (%s, %s) AND user_2 IN (%s, %s)",
                         (user_id, friend_id, user_id, friend_id))
        ...
        log_user_action(user_id, callback_query.from_user.username,
                        "rejection", f"rejected friend request from @{friend_name}")

    except Exception as e:
        error_traceback = traceback.format_exc()
        log_error(callback_query.from_user.id,
                  f"Error in reject_friend: {e}", error_traceback)
        await bot.send__message(callback_query.from_user.id,
